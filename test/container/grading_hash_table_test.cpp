...
void InsertTestCall(KeyType k /* unused */, ValueType v /* unused */, KeyComparator comparator) {
  ...
  LinearProbeHashTable<KeyType, ValueType, KeyComparator> ht("blah", bpm, comparator, 1000, HashFunction<KeyType>());
  ...
  std::vector<ValueType> res;
  ht.GetValue(nullptr, key20, &res);
  EXPECT_EQ(0, res.size());

  for (int i = 20; i < 65; i++) {
    auto value = GetValue<ValueType>(i);
    ht.Insert(nullptr, key20, value);
    std::vector<ValueType> res;
    ht.GetValue(nullptr, key20, &res);
...
template <typename KeyType, typename ValueType, typename KeyComparator>
void ResizeTestCall(KeyType k /* unused */, ValueType v /* unused */, KeyComparator comparator) {
  auto *disk_manager = new DiskManager("test.db");
  auto *bpm = new BufferPoolManager(50, disk_manager);
  LinearProbeHashTable<KeyType, ValueType, KeyComparator> ht("blah", bpm, comparator, 1000, HashFunction<KeyType>());
...
TEST(HashTableTest, ResizeTest) {
  ResizeTestCall(1, 1, IntComparator());

  GenericTestCall<GenericKey<8>, RID, GenericComparator<8>>(ResizeTestCall);
  GenericTestCall<GenericKey<16>, RID, GenericComparator<16>>(ResizeTestCall);
  GenericTestCall<GenericKey<32>, RID, GenericComparator<32>>(ResizeTestCall);
  GenericTestCall<GenericKey<64>, RID, GenericComparator<64>>(ResizeTestCall);
}
...
TEST(HashTableTest, IntegratedConcurrencyTest) {
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    LinearProbeHashTable<int, int, IntComparator> *ht =
        new LinearProbeHashTable<int, int, IntComparator>("blah", bpm, IntComparator(), 1000, HashFunction<int>());
...
TEST(HashTableTest, ResizeConcurrencyTest) {
  const int num_threads = 5;
  const int num_runs = 50;
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    LinearProbeHashTable<int, int, IntComparator> *ht =
        new LinearProbeHashTable<int, int, IntComparator>("blah", bpm, IntComparator(), 100, HashFunction<int>());
    std::vector<std::thread> threads(num_threads);
