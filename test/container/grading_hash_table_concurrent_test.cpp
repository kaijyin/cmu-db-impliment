...
void InsertHelper(LinearProbeHashTable<int, int, IntComparator> *hash_table, const std::vector<int> &keys, uint64_t tid,
...
void InsertHelperSplit(LinearProbeHashTable<int, int, IntComparator> *hash_table, const std::vector<int> &keys,
...
void DeleteHelper(LinearProbeHashTable<int, int, IntComparator> *hash_table, const std::vector<int> &remove_keys,
...
void DeleteHelperSplit(LinearProbeHashTable<int, int, IntComparator> *hash_table, const std::vector<int> &remove_keys,
                       int total_threads, uint64_t tid, __attribute__((unused)) uint64_t thread_itr) {
  for (auto key : remove_keys) {
    if (static_cast<uint64_t>(key) % total_threads == thread_itr) {
      int value = key;
...
void LookupHelper(LinearProbeHashTable<int, int, IntComparator> *hash_table, const std::vector<int> &keys, uint64_t tid,
                  __attribute__((unused)) uint64_t thread_itr = 0) {
  for (auto key : keys) {
    int value = key;
...
void InsertTest1Call() {
  for (size_t iter = 0; iter < NUM_ITERS; iter++) {
    ...
    LinearProbeHashTable<int, int, IntComparator> hash_table("foo_pk", bpm, IntComparator(), 1000, HashFunction<int>());
